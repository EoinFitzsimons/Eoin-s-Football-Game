...
export class MatchEngine {
  ...
  update() {
    ...
    // Goal detection: check if ball crosses left or right edge within goal area
    const goalTop = this.height / 2 - 40;
    const goalBottom = this.height / 2 + 40;
    ...
    if (this.ball.x >= this.width && this.ball.y >= goalTop && this.ball.y <= goalBottom) {
      this.score[0]++;
      this.resetBall();
    }
    this.time++;
