...
export class League {
  ...
  // Factory method to create a sample league with teams and players
  static createSampleLeague(numTeams = 8, playersPerTeam = 18) {
    const teamNames = [
      'Red FC', 'Blue United', 'Green Town', 'Yellow Stars',
      'Purple City', 'Orange County', 'White Wolves', 'Black Panthers',
      'Silver Foxes', 'Golden Eagles', 'Aqua Sharks', 'Crimson Kings'
    ];
    const teams = [];
    for (let i = 0; i < numTeams; i++) {
      teams.push(Team.createSampleTeam(teamNames[i % teamNames.length], playersPerTeam));
    }
    return new League({ name: 'Premier League', teams });
  }
